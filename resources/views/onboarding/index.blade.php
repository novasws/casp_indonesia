...
.agent-offline{display:inline-flex;align-items:center;gap:4px;font-size:.7rem;color:#94A3B8;font-weight:500;margin-bottom:8px}
.agent-offline::before{content:'●';font-size:.55rem;color:#94A3B8}
...
.agent-jadwal{font-size:.7rem;color:var(--gray-500);margin-top:6px;font-weight:600;background:var(--gray-100);padding:2px 6px;border-radius:4px;display:inline-block}
/* Paket */
...
.form-group label{display:block;font-size:.85rem;font-weight:600;color:var(--gray-700);margin-bottom:6px;letter-spacing:.02em}
.form-group input,.form-group select,.form-group textarea{display:block;width:100%;padding:.65rem 1rem;font-size:.95rem;font-weight:400;line-height:1.5;color:var(--gray-800);background-color:#F8FAFC;border:1px solid var(--gray-300) !important;border-radius:0px !important;transition:all .2s ease}
.form-group input:focus,.form-group select:focus,.form-group textarea:focus{background-color:#fff;border-color:var(--blue-600) !important;outline:0;box-shadow:0 0 0 3px rgba(30,94,191,.15) !important}
.form-check{display:flex;align-items:flex-start;gap:10px;margin-bottom:24px;margin-top:8px}
.form-check input[type=checkbox]{width:18px;height:18px;margin-top:2px;cursor:pointer;accent-color:var(--blue-600)}
.form-check label{font-size:.85rem;color:var(--gray-600);line-height:1.6;cursor:pointer}
.form-check a{color:var(--blue-600);font-weight:600;text-decoration:none}
.form-check a:hover{text-decoration:underline;color:var(--blue-800)}
.btn-submit{display:block;width:100%;padding:.8rem 1.5rem;font-size:1rem;font-weight:700;letter-spacing:0.02em;color:#fff !important;background-color:var(--blue-600) !important;border:1px solid var(--blue-600) !important;border-radius:0px !important;text-align:center;cursor:pointer;transition:all .2s ease;margin-top:24px;box-shadow:0 4px 12px rgba(30,94,191,.2)}
.btn-submit:hover{background-color:var(--blue-800) !important;border-color:var(--blue-800) !important;transform:translateY(-1px);box-shadow:0 6px 16px rgba(30,94,191,.3)}
.btn-submit:active{transform:translateY(0)}
.btn-back{display:block;width:100%;padding:.8rem 1.5rem;font-size:.95rem;font-weight:600;color:var(--gray-600) !important;background-color:transparent !important;border:1px solid var(--gray-300) !important;border-radius:0px !important;text-align:center;cursor:pointer;transition:all .2s ease;margin-top:12px}
.btn-back:hover{color:var(--gray-900) !important;background-color:var(--gray-50) !important;border-color:var(--gray-400) !important}
.err-box{display:none;background:#FEF2F2;border:1px solid #FECACA;border-radius:4px;padding:12px 16px;margin-bottom:20px;font-size:.85rem;color:#991B1B;font-weight:500}
@media(max-width:600px){
    .onboard-body{padding:16px 16px}
    .progress-bar{padding:16px; overflow-x:auto;}
    .progress-steps{width:max-content; gap:24px; padding-bottom:8px}
    .agent-grid,.paket-grid{grid-template-columns:1fr}
    .pay-grid{grid-template-columns:1fr}
    .form-row{grid-template-columns:1fr; gap:16px}
    .onboard-card{padding:24px 16px; border:none; border-radius:0; box-shadow:none; border-top:1px solid var(--gray-200); border-bottom:1px solid var(--gray-200)}
    .onboard-wrap{background:white}
    .form-group input,.form-group select,.form-group textarea{font-size:1rem; padding:0.75rem 1rem;}
}
</style>
@endpush
    ...
    <div class="onboard-wrap">
        ...
        <div class="onboard-body">
            ...
            <div id="step1" class="onboard-card fade-in">
                ...
                <div class="form-group">
                    ...
                    <select id="s1-bidang">
                        @foreach($bidang_hukum as $b)
                            <option {{ request('bidang') == $b ? 'selected' : '' }}>{{ $b }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label>Deskripsi Keluhan / Masalah *</label>
                    <textarea id="s1-keluhan" rows="4" placeholder="Jelaskan secara singkat permasalahan hukum yang sedang Anda hadapi..."></textarea>
                </div>
                ...
                <button class="btn-back" onclick="window.location.href='{{ route('landing') }}'">← Batal / Kembali ke Beranda</button>
            </div>

            {{-- ===== STEP 2: PILIH AGENT ===== --}}
            <div id="step2" class="onboard-card fade-in" style="display:none">
                <h2>Pilih Konsultan Hukum</h2>
                <p>Hanya konsultan yang terjadwal & aktif yang bisa dipilih.</p>
                ...
                <div class="agent-grid">
                    ...
                    @php 
                        $isOnline = $k->status === 'online'; 
                        $imgName = 'admin' . ($index + 1) . '.jpg';
                        $hasImg = file_exists(public_path('images/' . $imgName));
                    @endphp
                    <div class="agent-card {{ !$isOnline ? 'offline' : '' }}"
                         id="agent-{{ $k->id }}"
                         style="{{ !$isOnline ? 'opacity:0.8;' : '' }}"
                         onclick="selectAgent({{ $k->id }}, '{{ addslashes($k->nama_lengkap) }}', {{ $isOnline ? 'true' : 'false' }})">
                        @if($k->foto)
                            <img src="{{ asset('storage/' . $k->foto) }}" alt="{{ $k->nama_lengkap }}" class="agent-avatar object-cover border-4 border-slate-50 shadow-sm">
                        @elseif($hasImg)
                            <img src="{{ asset('images/' . $imgName) }}" alt="{{ $k->nama_lengkap }}" class="agent-avatar object-cover border-4 border-slate-50 shadow-sm">
                        @else
                        ...
                        @else
                            <div class="agent-offline">Offline</div>
                        @endif
                        <div class="agent-name">{{ $k->nama_lengkap }}</div>
                        <div class="agent-spec">{{ $k->spesialisasi }}</div>
                        <div class="agent-jadwal">Shift: {{ $k->jadwal_shift ?? 'Belum Diatur' }}</div>
                    </div>
                    @endforeach
                </div>
                <div id="jadwal-section" class="form-group" style="display:none; margin-bottom: 24px; background: #eff6ff; padding: 16px; border-radius: 12px; border: 1px solid #bfdbfe;">
                    <label style="color: #1e3a8a; font-weight: 700; margin-bottom: 4px;">Konsultan Sedang Offline</label>
                    <p style="font-size: 0.8rem; color: #3b82f6; margin-bottom: 0;">Sesi Anda akan otomatis dimasukkan pada antrean shift kerja agen berikutnya. Anda bisa menunggu ruangan obrolan hingga agen bersangkutan login.</p>
                </div>
                <button class="btn-submit" id="btn-lanjut-step2" onclick="nextStep(2)">Lanjutkan →</button>
...
let onlineAgents = @json($konsultan->map->only(['id', 'nama_lengkap', 'status']));
let selectedAgent = onlineAgents.length > 0 ? onlineAgents[0].id : null;
let selectedAgentNama = onlineAgents.length > 0 ? onlineAgents[0].nama_lengkap : '';
let selectedAgentOnline = onlineAgents.length > 0 ? (onlineAgents[0].status === 'online') : true;
let selectedPaket = 2;
...
async function nextStep(s) {
    ...
    if (s === 1) {
        ...
        const keluhan = document.getElementById('s1-keluhan').value.trim();
        const agree   = document.getElementById('s1-agree').checked;

        if (!nama || !hp || !email || !keluhan) { showErr('err1', 'Harap isi semua field yang wajib (*).'); return; }
        ...
        try {
            const res  = await fetch('{{ route('onboarding.step1') }}', {
                ...
                body: JSON.stringify({ nama, hp, email, bidang, keluhan }),
            });
    ...
    if (s === 2) {
        if (!selectedAgent) { showErr('err2', 'Mohon pilih salah satu konsultan terlebih dahulu.'); return; }
        
        if (!selectedAgentOnline) {
            // We just submit it without explicit date, it gets queued.
        }
...
// =============== AGENT ===============
function selectAgent(id, nama, isOnline) {
    document.querySelectorAll('.agent-card').forEach(c => c.classList.remove('selected'));
    document.getElementById('agent-' + id).classList.add('selected');
    selectedAgent     = id;
    selectedAgentNama = nama;
    selectedAgentOnline = isOnline;

    const jSection = document.getElementById('jadwal-section');
    const btnLanjut = document.getElementById('btn-lanjut-step2');
    if (!isOnline) {
        jSection.style.display = 'block';
        if (btnLanjut) btnLanjut.textContent = 'Masuk Antrean & Lanjutkan →';
    } else {
        jSection.style.display = 'none';
        if (btnLanjut) btnLanjut.textContent = 'Lanjutkan →';
    }
}
...
updatePaySummary();
if(selectedAgent) {
    // Initial call to show/hide UI correctly
    const kData = onlineAgents.find(a => a.id == selectedAgent);
    if(kData) selectAgent(selectedAgent, kData.nama_lengkap, kData.status === 'online');
}
</script>
@endpush
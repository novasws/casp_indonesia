...
@section('title', 'Ruang Chat - Admin CASP Indonesia')
@section('page_title', 'Ruang Percakapan Konsultasi')
...
@php
    $isAudit = auth()->user()->is_superadmin && $konsultasi->konsultan_id != auth()->id();
@endphp
...
<div class="flex flex-col h-[calc(100vh-4rem)] -m-6 md:-m-8 overflow-hidden bg-slate-50 relative z-20">
    ...
    <div class="mobile-back-btn bg-white px-4 py-3 border-b border-slate-200 items-center gap-3 shrink-0 relative z-10">
        ...
        <div>
            <h3 class="text-sm font-bold text-slate-800">{{ $konsultasi->klien_nama }}</h3>
            <p class="text-xs text-slate-500">Terhubung via {{ $konsultasi->paket }}</p>
        </div>
    </div>

    {{-- Top Info Bar --}}
    <div class="bg-white px-6 py-4 border-b border-slate-200 shadow-sm flex items-center justify-between shrink-0 relative z-10 hidden"> 
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.konsultasi.index') }}" class="w-10 h-10 rounded-full bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 hover:text-brand-600 hover:bg-brand-50 transition-colors">
                <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
            </a>
            <div>
                <h3 class="text-lg font-bold text-slate-800">{{ $konsultasi->klien_nama }}</h3>
                <div class="text-xs font-semibold text-brand-600">Terhubung via {{ $konsultasi->paket }}</div>
            </div>
        </div>
        <div class="flex items-center gap-3">
            @if($konsultasi->status == 'berjalan')
            ...
            <div class="flex items-center gap-2">
                <div class="px-3 py-1.5 bg-rose-50 text-rose-600 border border-rose-100 flex items-center gap-2 shadow-sm rounded-sm" title="Sisa Waktu Konsultasi">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    <span id="countdownTimer" class="text-xs font-bold tracking-wider">--:--:--</span>
                </div>
                <form action="{{ route('admin.konsultasi.akhiri', $konsultasi->id) }}" method="POST" onsubmit="return confirm('Yakin ingin mengakhiri sesi ini secara permanen?')">
                    @csrf
                    <button type="submit" class="px-4 py-1.5 bg-brand-900 text-white rounded-[3px] text-xs font-bold hover:bg-brand-800 transition-colors shadow-sm">Akhiri Sesi</button>
                </form>
            </div>
            @endif
        </div>
    ...
    @if($isAudit)
    <div class="bg-amber-100 border-b border-amber-200 px-6 py-2 shrink-0 z-10 flex items-center justify-center gap-2 shadow-sm relative">
        <svg class="w-4 h-4 text-amber-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
        <span class="text-xs font-bold text-amber-800 tracking-wide uppercase">Mode Audit: Anda memantau percakapan ini sebagai Superadmin.</span>
    </div>
    @endif
    ...
    <div class="bg-white border-b border-slate-200 shrink-0 z-10 relative">
        <button onclick="toggleDetail()" class="w-full px-6 py-3 flex items-center justify-between text-left hover:bg-slate-50 transition-colors group">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="text-sm font-bold text-slate-800">Detail Masalah Klien</h4>
                    <p class="text-xs text-slate-500 font-medium">Bidang: <span class="text-brand-600">{{ $konsultasi->bidang_hukum ?: 'Tidak disebutkan' }}</span></p>
                </div>
            </div>
        ...
        <div id="detail-content" class="px-6 pb-5 pt-1 hidden">
            <div class="bg-slate-50 rounded-lg p-4 border border-slate-200 shadow-inner relative max-h-48 overflow-y-auto">
                <h5 class="text-[10px] font-bold text-slate-500 mb-2 uppercase tracking-widest">Deskripsi Keluhan:</h5>
                <p class="text-sm text-slate-700 leading-relaxed whitespace-pre-wrap">{{ $konsultasi->deskripsi_keluhan ?: 'Klien tidak menyertakan deskripsi keluhan tambahan.' }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="relative z-10 space-y-6 max-w-4xl mx-auto flex flex-col min-h-full" id="chat-messages">
            ...
            <div class="text-center mt-auto pt-8">
                <span class="px-3 py-1 bg-white border border-slate-200 text-slate-400 text-xs font-semibold rounded-full shadow-sm">
                    Sesi dibuka pada {{ $konsultasi->created_at->format('d M Y, H:i') }}
                </span>
            </div>
...
<script>
    ...
    const NAMA_KLIEN  = "{{ $konsultasi->klien_nama }}";
    const IN_KLIEN    = "{{ substr($konsultasi->klien_nama, 0, 2) }}";
    const URL_FETCH   = '{{ route('admin.konsultasi.fetchPesan', $konsultasi->id) }}';
    const URL_KIRIM   = '{{ route('admin.konsultasi.reply', $konsultasi->id) }}';
    const CSRF        = document.querySelector('meta[name="csrf-token"]').content;

    let renderedIds = [];
    let sisaDetik   = {{ $konsultasi->sisa_detik ?? 0 }};

    // --- Timer Countdown ---
    const timerDisplay = document.getElementById('countdownTimer');
    if(timerDisplay && sisaDetik > 0) {
        // Initial display
        ...
        timerDisplay.textContent = formatTime(sisaDetik);
        ...
        const updateTimer = setInterval(() => {
            ...
            if(sisaDetik <= 0) {
                ...
                timerDisplay.parentElement.classList.replace('text-rose-600', 'text-slate-500');
                timerDisplay.parentElement.classList.replace('bg-rose-50', 'bg-slate-100');
                timerDisplay.parentElement.classList.replace('border-rose-100', 'border-slate-200');
                return;
            }
            timerDisplay.textContent = formatTime(sisaDetik);
        }, 1000);
    ...
    function appendMessage(p) {
        ...
        if(p.pengirim === 'klien') {
            ...
            div.innerHTML = `
                <div class="w-10 h-10 shrink-0 rounded-full bg-slate-200 flex items-center justify-center text-slate-600 font-bold border border-slate-300 shadow-sm text-sm">
                    ${escapeHTML(IN_KLIEN)}
                </div>
            ...
            div.innerHTML = `
                <div class="bg-brand-900 p-4 rounded-2xl rounded-tr-sm max-w-[80%] shadow-[0_4px_15px_-5px_rgba(6,22,46,0.3)] relative group text-left">
                    <h4 class="text-xs font-bold text-brand-300 mb-1 flex justify-end items-center gap-2">
                        <span class="text-brand-400 font-normal">${p.waktu}</span> Anda (Konsultan)
                    </h4>
